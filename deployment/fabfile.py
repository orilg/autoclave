...
def deploy_vagrant():
    os.chdir(os.path.abspath(os.path.join(_PROJECT_PATH, 'config')))
    local("vagrant up")
...
def deploy():
    ...
    with cd("/tmp"): # various commands might fail saving data to /root...
        ...
        _deploy_run_as_autoclave_user("{0}/env/bin/pip install -r {0}/src/config/pip_requirements.txt".format(config.deployment.root_path)
    ...
    require.supervisor.process(config.app.CELERY_WORKER_SERVICE_NAME,
        command="{0}/env/bin/celeryd -B --config=celery.config".format(config.deployment.root_path),
        directory=config.deployment.src_path,
        user=config.deployment.user,
        )
...
def _deploy_sync_project():
    ...
    _deploy_run_as_autoclave_user("rsync -rv {}/ {}/".format(tmp_dir, config.deployment.src_root))

def _deploy_run_as_autoclave_user(cmd):
    sudo(cmd, user=config.deployment.user)

def _generate_testing_nginx_configuration():
    return configuration_templates.nginx.render(tcp_port=config.deployment.web.testing_frontend_port,
                                                static_root=config.deployment.web.static_root,
                                                daemon=False, config=config)
def _generate_production_nginx_configuration(installation_root):
    return configuration_templates.nginx.render(tcp_port=config.app.DEPLOYMENT_FRONTEND_TCP_PORT,
                                                static_root="{}/src/web/static/static".format(installation_root),
